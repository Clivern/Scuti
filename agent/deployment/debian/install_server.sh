...
[Service]
ExecStart=/etc/mandrill/mandrill server -c /etc/mandrill/config.prod.yml
Restart=on-failure
RestartSec=2

[Install]
WantedBy=multi-user.target" > /etc/systemd/system/mandrill_server.service

    systemctl daemon-reload
    systemctl enable mandrill_server.service
    systemctl start mandrill_server.service
